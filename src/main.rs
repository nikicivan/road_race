...
fn main() {
    ...
    // Start some the background music
    game.audio_manager
...
fn game_logic(engine: &mut Engine, game_state: &mut GameState) {
    ...
    for event in engine.collision_events.drain(..) {
        // We don't care if obstacles collide each other or if a collision ended
    ...
    // Detect ig Game is over
    if game_state.health_amount == 0 {
        game_state.lost = true;
        let game_over = engine.add_text("game over", "Game Over");
        game_over.font_size = 128.0;
        engine.audio_manager.stop_music();
        engine.audio_manager.play_sfx(SfxPreset::Jingle3, 0.3);
    }
}
